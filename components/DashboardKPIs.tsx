...
export const DashboardKPIs: React.FC<Props> = ({ history, planningData, inventory, movements, products, onStoreClick }) => {
  const kpiData = useMemo(() => {
    ...
    const overstocked = storeRisks.filter(s => s.cover > 120).slice(0, 3);
    const understocked = storeRisks.filter(s => s.cover < 15).reverse().slice(0, 3);
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto max-h-[200px] pr-1">
           {kpiData.understocked.length > 0 && (
             <div>
                <p className="text-xs font-bold text-red-600 uppercase mb-2">Potential Stock Outs</p>
                <div className="space-y-2">
                   {kpiData.understocked.map(s => (
                      <div key={s.name} className="flex justify-between text-xs bg-red-50 p-2 rounded border border-red-100 cursor-pointer hover:bg-red-100" onClick={() => onStoreClick?.(s.name)}>
                         <span className="truncate max-w-[140px] text-red-800 font-medium">{s.name}</span>
           ...
           {kpiData.overstocked.length > 0 && (
             <div>
                <p className="text-xs font-bold text-amber-600 uppercase mb-2">Overstocked Stores</p>
                <div className="space-y-2">
                   {kpiData.overstocked.map(s => (
                      <div key={s.name} className="flex justify-between text-xs bg-amber-50 p-2 rounded border border-amber-100 cursor-pointer hover:bg-amber-100" onClick={() => onStoreClick?.(s.name)}>
                         <span className="truncate max-w-[140px] text-amber-800 font-medium">{s.name}</span>
                         <span className="font-bold text-amber-700">{s.cover.toFixed(0)} days</span>
                      </div>
                   ))}
                </div>
             </div>
